...
> **Status:** implemented and shipping (v1.0.x). Reflects the built system;
...
keeps a persistent per-domain cookie jar so a session established once is reused
by later automated calls, and exposes a live VNC view so an operator can
complete an interactive check by hand from the mycelium dashboard when
automation can't. See `DISCLAIMER.md` for acceptable use.
...
| `POST /v1/session/start` | `{url, proxy_url?, egress?}` | `{id, target_url}` (409 if one open) |
| `POST /v1/session/{id}/close` | — | `{ok}` — persists jar, tears down x11vnc |
| `GET  /v1/session/current` | — | `{id, target_url}` (`id:""` if none) |
| `GET  /v1/session/{id}/vnc-ws` | — | raw VNC websocket |
| `GET  /vnc/*` | — | vendored noVNC static (`vnc_lite.html`) |
...
interactive stack at all. `browser_engine = "none"` in config goes further:
...
WORKDIR /src
COPY . .
# FEATURES overridable: e.g. --build-arg FEATURES="flaresolverr" for a no-VNC image,
# or FEATURES="vnc,flaresolverr,ytdlp" + --build-arg WITH_YTDLP=1 to add yt-dlp.
...
# (drop xvfb x11vnc when building without the vnc feature)
...
`entrypoint`/`cobweb` starts Xvfb itself (when `vnc`) so there's one supervised
process tree; `tini` reaps the browser/x11vnc children.
