# Multi-stage build.
#
#   docker build -t cobweb:latest .                        # flaresolverr (default)
#   docker build --build-arg FEATURES="" \
#                -t cobweb:lean .                           # fast-path only
#   docker build --build-arg FEATURES="flaresolverr,ytdlp" \
...
# The default image does NOT carry it. There used to be a `vnc` feature/build
# arg (an interactive Cloudflare/Turnstile solver over noVNC) — removed
# entirely, not just defaulted off; see Cargo.toml.
