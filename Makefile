...
#
# Default features (vnc/flaresolverr) are on. `ytdlp` is opt-in:
...
# wreq's BoringSSL (btls) needs cmake+clang; chromium/xvfb/x11vnc for the browser+vnc tests.
PREP        = apt-get update -qq && apt-get install -y -qq cmake clang chromium fonts-liberation xvfb x11vnc >/dev/null 2>&1; \
