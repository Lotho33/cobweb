...
/// (`RuntimeSettings::nav_timeout_ms` — `src/settings.rs`, live-tunable via
/// `PATCH /v1/settings`). Either way the result is clamped to
...
pub struct ResolveRequest {
    ...
    /// Block tracker/ad domains loaded from `[blocklist]` (independent of
    /// `block_resources`). Defaults to `true` — a no-op unless `[blocklist]`
    /// is `enabled` and has patterns loaded.
    #[serde(default)]
