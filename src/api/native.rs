...
pub async fn resolve(
    ...
    let params = ResolveParams {
        ...
        timeout,
        block_resources: req.block_resources.unwrap_or(true),
    };
...
pub async fn navigate(
    ...
    let url = parse_url(&req.url)?;
    let timeout = req.timeout();
    let r = pipeline::navigate_fastpath(
        &st,
        url,
        req.egress.clone(),
        req.proxy_url.clone(),
        req.referer.clone(),
        req.extra_headers.clone(),
        timeout,
    )
    .await?;
...
pub async fn eval(State(st): State<AppState>, Json(req): Json<EvalRequest>) -> Result<Json<Value>> {
    let url = parse_url(&req.url)?;
    let timeout = req.timeout();
    let v = pipeline::browser_eval(
        &st,
        url,
        req.js.clone(),
        req.egress.clone(),
...
pub async fn session_start(
    ...
    let info = mgr
        ...
        .await?;
    Ok(Json(
        json!({ "id": info.id, "target_url": info.target_url }),
    ))
}
...
pub async fn jar_list(State(st): State<AppState>) -> Json<Value> {
    Json(json!(st.jar.list().await))
}
