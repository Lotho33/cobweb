...
impl CdpEngine {
    pub fn new(cfg: BrowserConfig, max_contexts: usize, idle_shutdown_secs: u64) -> Self {
        Self {
            cfg,
            max_contexts: max_contexts.max(1),
            idle_after: Duration::from_secs(idle_shutdown_secs),
    ...
    async fn launched(&self) -> BrowserResult<(Arc<CdpClient>, String)> {
        ...
        if g.is_none() {
            ...
            tracing::info!(headless = chromium.headless, ua = %default_ua, "Chromium + CDP ready");
            *g = Some(Launched {
                chromium,
                client,
                default_ua,
    ...
    /// Spawn (once) the task that tears down Chromium + Xvfb after
    /// `idle_after` with no contexts in use. `idle_after == 0` disables it.
    fn start_reaper(&self) {
        if self.idle_after.is_zero() || self.reaper_started.swap(true, Ordering::SeqCst) {
            return;
        }
        ...
        tokio::spawn(async move {
            let tick = Duration::from_secs(15).min(idle_after / 2 + Duration::from_secs(1));
            loop {
                tokio::time::sleep(tick).await;
                if in_use.load(Ordering::SeqCst) > 0 {
                    continue;
                }
                if last_used.lock().unwrap().elapsed() < idle_after {
                    continue;
                }
                let taken = {
                    ...
                    if in_use.load(Ordering::SeqCst) > 0
                        || last_used.lock().unwrap().elapsed() < idle_after
                    {
                        None
                    } else {
                        g.take()
                ...
                if let Some(l) = taken {
                    tracing::info!("browser idle: tearing down Chromium + Xvfb");
...
impl BrowserEngine for CdpEngine {
    ...
    async fn display(&self) -> Option<String> {
        self.launched
            ...
            .await
            .as_ref()
            .and_then(|l| l.chromium.xvfb_display().map(str::to_string))
    }
...
impl CdpContext {
    ...
    async fn setup(&mut self, opts: &ContextOptions) -> BrowserResult<()> {
        ...
        let block_fut = async {
            if opts.block_resources {
                self.call("Network.setBlockedURLs", json!({ "urls": BLOCKED_URLS }))
                    .await
                    .map(|_| ())
            } else {
                Ok(())
    ...
    /// instrumentation as the main page — including the UA — then let it run.
    async fn adopt_child(&self, child_sid: &str) {
        ...
        if self.block_resources {
            let _ = self
                .client
                .call("Network.setBlockedURLs", json!({ "urls": BLOCKED_URLS }), s)
                .await;
        }
        let _ = self
            .client
...
impl BrowserContext for CdpContext {
    ...
    async fn sniff(
        ...
        sessions.insert(self.session_id.clone());
        ...
        loop {
            ...
            if e.method == "Target.attachedToTarget" {
                ...
                if ttype == "page" || ttype == "tab" {
                    ...
                    sessions.insert(child.to_string());
            ...
            match e.method.as_str() {
                "Network.requestWillBeSent" => {
                    ...
                    if let Some(id) = req_id {
                        ...
                        if req_type_trackable(rtype)
                            && (reqs.contains_key(&id) || reqs.len() < MAX_TRACKED_REQS)
                        {
        ...
        tracing::debug!(sessions = sessions.len(), "sniff gave up with no match");
