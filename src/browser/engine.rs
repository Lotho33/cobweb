...
pub enum BrowserError {
    /// Engine could not start (no Chromium binary, spawn failed, …).
...
pub struct ContextOptions {
    ...
    /// Block tracker/ad domains from `[blocklist]` (independent of
    /// `block_resources` — see `src/blocklist.rs`). A no-op if `[blocklist]`
    /// isn't `enabled` or has no patterns loaded yet.
...
pub trait BrowserEngine: Send + Sync {
    ...
    /// Tear down Chromium (idle shutdown / process exit).
    async fn shutdown(&self);

    /// Live-update the idle-shutdown threshold (seconds; 0 disables it),
    /// pushed from `PATCH /v1/settings` (`src/settings.rs`). Default no-op —
    /// only `CdpEngine` has a reaper to tune; `MockEngine` doesn't need one.
    fn set_idle_shutdown_secs(&self, _secs: u64) {}
}
