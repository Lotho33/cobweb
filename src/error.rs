...
pub enum CobwebError {
    ...
    /// Landed on a challenge and no automated tier could get past it (no
    /// browser sniff success, no FlareSolverr configured or it also
    /// failed). Terminal — cobweb has no manual-solve capability of its
    /// own; carries the domain so a caller can decide what to do (e.g.
    /// point [flaresolverr].endpoint at an instance and retry).
    #[error("challenge on `{domain}` could not be resolved automatically")]
