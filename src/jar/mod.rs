...
pub struct Jar {
    root: PathBuf,
    /// Seconds. Live-tunable via `PATCH /v1/settings`
    /// (`RuntimeSettings::jar_default_ttl_secs` pushes into
    /// [`Jar::set_default_ttl_secs`]) — an atomic instead of a plain
    /// `Duration` field so a change takes effect immediately, with no
    /// signature change to `new`/`default_ttl`/`is_stale`/`get_fresh`.
    default_ttl_secs: AtomicU64,
    fail_streak_limit: AtomicU32,
...
impl Jar {
    ...
    /// Pushed from `PATCH /v1/settings` (`src/api/settings.rs`) — affects
    /// only jar entries created *after* the change; entries already on disk
    /// keep their own `ttl_hint_secs`.
    pub fn set_default_ttl_secs(&self, secs: u64) {
        self.default_ttl_secs.store(secs, Ordering::Relaxed);
    }

    /// Pushed from `PATCH /v1/settings`.
    pub fn set_fail_streak(&self, n: u32) {
        self.fail_streak_limit.store(n.max(1), Ordering::Relaxed);
    }
    ...
    pub fn is_stale(&self, entry: &JarEntry) -> bool {
        Utc::now() >= entry.expiry()
            || entry.fail_streak >= self.fail_streak_limit.load(Ordering::Relaxed)
    }
