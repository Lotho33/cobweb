...
/// single long-lived connection — a manual VNC session's websocket bridge has
/// no idle timeout of its own by design, or a `/v1/fetch` streaming a large
/// body — can wedge a restart/redeploy indefinitely: `axum`'s graceful
...
/// outright rather than waiting forever, so a stuck VNC bridge or a very long
/// download can't block teardown of the Chromium/Xvfb children that follow.
        ...
        _ = async move {
            ...
            tracing::warn!(
                ...
                "shutdown grace period elapsed with connections still open; forcing the \
                 listener closed so teardown (Chromium/Xvfb) can proceed"
            );
            Ok(())
        }
    }
