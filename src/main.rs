...
async fn run() -> ExitCode {
    ...
    if cli.check {
        ...
        println!(
            "  blocklist.state  : {} (managed via GET/PATCH /v1/blocklist)",
            cfg.blocklist.state_path.display()
        );
        println!(
            "  settings.state   : {} (DNS/timeouts, managed via GET/PATCH /v1/settings)",
            cfg.settings.state_path.display()
        );
    ...
    // Master on/off + sources are runtime state (`GET/PATCH /v1/blocklist`),
    // not config — `refresh`/the periodic loop are both cheap no-ops when
    ...
    // Tear down Chromium so nothing is orphaned.
