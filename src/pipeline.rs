...
//! "chromium"`). Tier 3b (FlareSolverr) and Tier 4 (VNC) still return a
//! structured outcome so the HTTP contract is already final.
...
pub enum Mode {
    ...
    Fast,
    /// Full pipeline, stopping before the manual VNC solve.
    #[default]
    Auto,
...
async fn resolve_inner(state: &AppState, params: ResolveParams) -> Result<ResolveOutcome> {
    ...
    // Tier 4 (manual VNC) is never entered automatically — see finish_unresolved.
    finish_unresolved(
        &domain,
        params.mode,
        last_reason,
