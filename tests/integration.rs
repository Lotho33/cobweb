...
async fn browser_endpoints_need_the_engine_when_it_is_off() {
    ...
    // A manual solve also needs the engine.
    let (status, body) = app
        .post("/v1/session/start", json!({ "url": "https://x/" }))
        .await;
    #[cfg(feature = "vnc")]
    {
        assert_eq!(status, 422, "body: {body}");
        assert_eq!(body["kind"], json!("needs_browser"));
    }
    #[cfg(not(feature = "vnc"))]
    {
        assert_eq!(status, 501, "body: {body}");
        assert_eq!(body["kind"], json!("not_implemented"));
    }

    let (status, body) = app.get("/v1/session/current").await;
    assert_eq!(status, 200);
